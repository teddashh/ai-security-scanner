...
> **專案狀態：**`v0.1.7` 是經過完整實機操作流程調整的 Windows 優先公開測試版。設定更簡單，網路掃描可直接輸入 IP 或網域，進度更可靠，掃描停止時也會清楚說明發生什麼事。
...
`v0.1.7` 是預定 `v0.2.0` 的 Windows 優先公開實測版，也完整納入 Candidate 11 在 Windows 實際操作應用程式後整理出的修正。第一次開啟會先認出已具備的環境；準備好的掃描會從使用者剛才選的動作直接開始；外部與內部網路檢查都能輸入 IP 或網域；掃描剛啟動時產生的事件也不會從進度頁消失。若檢查停止，已保存的證據不會丟失，畫面會提供可執行的下一步，也能匯出去識別診斷紀錄。匯入專案時，若忽略規則無法被安全確認，檔案會保留而不是被悄悄省略。程式仍會等使用者明確按下「開始掃描」，才會連線到目標。
