...
`v0.1.4` 是預定 `v0.2.0` 的公開實測預覽版。它不再建立 0/0 的空掃描，會在保存掃描前先確認本機工具與雲端短效連線真的可用；每種使用情境只帶你完成真正需要的設定，能安全建議本機網段，也會把設定失敗整理成一個有用的下一步與可下載的去識別診斷檔。安裝程式必須先通過跨平台自動建置與全新主機資格驗證，GitHub 才會發布 pre-release。規格要求的 IAM 新手真人首次使用研究，以及研究之後的正式品質檢查與程式碼審查，仍保留在正式版流程。
