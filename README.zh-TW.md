...
**用一套桌面應用程式，檢查網站、外部 IP、內部系統、程式碼、基礎設施程式碼、雲端、容器與 Kubernetes。** 從「我想保護什麼」開始，不必先看懂一長串掃描器名稱；最後得到的是一份清楚、可以開始處理的問題清單。
...
> **專案狀態：**`v0.1.4` 是通往 `v0.2.0` 的可安裝公開預覽版，可實際測試大幅簡化後的家用網路、網站、外部 IP、程式碼、基礎設施程式碼、雲端、容器與 Kubernetes 掃描流程；正式 `v0.2.0` 仍保留給產品完成並經過測試後 QC 的版本。
...
首次設定會先驗證隨附執行環境與主機必要條件，確認後才下載以校驗碼鎖定的機器映像，再初始化不具管理員權限的私有隔離環境，並逐步顯示狀態。下載可取消及續傳。應用程式不會修改系統 `PATH` 或使用套件管理器。在 Windows 上，唯一的管理員操作是由使用者在標準 Windows 確認視窗中明確同意的一次性 WSL 安裝或更新。

在 Windows 上，只要按 **交給程式處理**，再同意一次 Windows 確認視窗。ai-security-scanner 只會用 Microsoft 的可信任 `wsl.exe` 執行固定的必要操作，完成檢查後自動繼續；程式不會看到或儲存管理員密碼。如果 Windows 需要重新開機，程式會在下載約 257 MB 的機器映像前停下並清楚告知。手動 Microsoft 指令仍可在 **其他方式** 中找到。
...
`v0.1.4` 是預定 `v0.2.0` 的公開實測預覽版。它不再建立 0/0 的空掃描，會在保存掃描前先確認本機工具，並原子保留精確掃描計畫所需的雲端短效能力；每種使用情境只帶你完成真正需要的設定，能安全建議本機網段，也會把設定失敗整理成一個有用的下一步與可下載的去識別診斷檔。安裝程式必須先通過跨平台自動建置與全新主機資格驗證，GitHub 才會發布 pre-release。規格要求的 IAM 新手真人首次使用研究，以及研究之後的正式品質檢查與程式碼審查，仍保留在正式版流程。
