...
> **專案狀態：**`v0.1.5` 是 Windows 優先的公開預覽版。第一次開啟就會自動準備掃描工具，不再反覆出現 Ready／Repair，掃描開始前與進行中都能看到清楚的事件紀錄。
...
`v0.1.5` 是預定 `v0.2.0` 的 Windows 優先公開實測預覽版。第一次開啟時，它會先確認電腦已經具備什麼，再自動準備本機掃描工具。若安裝包裡的元件真的有問題，不會再讓使用者陷入 Ready／Repair 的無限循環；畫面會說明原因並引導下載最新版安裝程式，同時保留既有案件。掃描開始前與進行中都有含時間的事件紀錄，會顯示準備狀態、目前執行的掃描器、正在等待什麼，以及最後一次保存的進度；需要深入排查時也能下載去識別診斷檔。程式仍會等使用者明確按下「開始掃描」，才會連線到目標。正式 QC 與程式碼審查仍依規劃留在完整產品流程實測之後進行。
