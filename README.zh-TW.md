...
**用一套桌面應用程式，檢查網站、外部 IP、內部系統、程式碼、雲端、容器與 Kubernetes。** 從「我想保護什麼」開始，不必先看懂一長串掃描器名稱；最後得到的是一份清楚、可以開始處理的問題清單。
...
> **專案狀態：**`v0.1.4` 是通往 `v0.2.0` 的可安裝公開預覽版，可實際測試大幅簡化後的家用網路、網站、外部 IP、程式碼、雲端、容器與 Kubernetes 掃描流程；正式 `v0.2.0` 仍保留給產品完成並經過測試後 QC 的版本。
