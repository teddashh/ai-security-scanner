...
## 不必先變成資安工具專家，也能找出該處理的問題

**用一套桌面應用程式，檢查網站、外部 IP、內部系統、AI 應用與 Agent、一般程式碼、基礎設施程式碼、雲端、容器與 Kubernetes。** 從「我想保護什麼」開始，不必先看懂一長串掃描器名稱；最後得到的是一份清楚、可以開始處理的問題清單。

`ai-security-scanner` 把成熟的開源資安工具整理成一條有人帶路的流程。它會依你的情境安排適合的檢查，把技術輸出整理成看得懂的下一步，集中保留證據，並在修復後幫你確認前後差異。

### 可以檢查什麼？

- **已經上線的網站或 API**：找出對外服務與已知弱點。
- **外部 IP 或網域**：看清楚從網際網路可以碰到哪些服務。
- **公司內部 IT 環境**：檢查選定的系統或設定快照。
- **正在開發的 AI 應用或 Agent**：檢查 vibe coding 與 AI 協作產生的軟體，並在適用結果上附上 AIDEFEND 參考座標。
- **一般程式碼**：在本機找高風險寫法與暴露的秘密，不會修改檔案。
- **基礎設施程式碼**：在部署前找出雲端與環境設定錯誤。
- **AWS、Azure、GCP 或 Microsoft 365**：盤點資產、設定、身分與權限。
- **容器映像**：找出有弱點的套件並產生軟體內容清單（SBOM）。
- **Kubernetes**：檢查工作負載設定與節點安全狀態。

### 從「該檢查什麼？」到「該修什麼？」只要三步

1. **選擇目的。** 挑一個最符合你現在需求的情境。
2. **加入目標。** 選取專案、輸入網站或 IP，或用唯讀方式連接雲端帳號。
3. **開始檢查。** 先看最重要的問題，需要時再打開完整證據；修好後重新執行，就能確認差異。

### 為什麼使用 ai-security-scanner？

- **一條清楚的流程：**不用自己安裝、研究、拼湊一整箱掃描工具。
- **結果說人話：**直接看懂問題、受影響的資產，以及下一個有用的動作。
- **資料由你保管：**掃描專案與詳細結果留在你的電腦，除非你主動匯出。
- **隨時知道目前進度：**清楚看見哪些已檢查、哪些還在等待、哪些需要更多資料。
- **框架對照不再擋住重點：**在適用結果上關聯 NIST CSF、ISO 27001 與 AIDEFEND，同時讓畫面專注在該修什麼。
- **方便分享，也方便確認修復：**匯出清楚的報告，修好後直接比較前後結果。

## 立即體驗引導流程

用準備好的範例結果，看看網站檢查如何一路變成有優先順序的改善清單。展示版直接在瀏覽器執行，現在就能走完整個產品流程。[開啟瀏覽器展示版操作方式](#瀏覽器展示版)。

## 想看細節？

<!-- Release line: v0.1.6. -->

> **專案狀態：**`v0.1.6` 是 Windows 優先的公開預覽版。它修正 `v0.1.5` 回報的網路掃描啟動失敗，會自動準備本機工具，也會清楚說明掃描器尚未執行前究竟卡在哪裡。
...
- [發布流程與證據](docs/release/README.md)

以下章節保留完整的操作與工程細節，但不再讓它們佔據產品介紹的第一層。
...
它**不會**保證組織絕對安全、不會取代合格的資安專業人員，也不會產生 ISO 27001、NIST 或 AIDEFEND 認證分數。框架對照只是方便討論問題的座標，不是合規結論。「沒有發現問題」也不等於「所有地方都檢查過」。

### 詳細使用情境

不需要先挑掃描器名稱。選一個最像你情況的入口即可。這張詳細表格會告訴你該準備什麼，以及每種檢查如何控制範圍：
...
| 正在開發的 AI 應用或 Agent | 明確標示為 AI 應用的本機唯讀專案或程式碼儲存庫快照 | 在本機檢查程式碼、秘密、相依套件與相關部署檔案，並在適用結果上附上所選 AIDEFEND 關聯座標；不上傳或修改檔案，也不宣稱符合框架。 |
| 自己寫的程式碼 | 本機唯讀專案或程式碼儲存庫快照 | 在本機找高風險寫法與秘密模式，在結果中遮罩找到的秘密值，而且不會修改專案檔案；不上傳程式碼，也不拿找到的秘密登入線上服務。 |
...
`v0.1.6` 是預定 `v0.2.0` 的 Windows 優先公開實測預覽版。網站、外部 IP、內部網路與其他需要連線的檢查，現在會讓掃描工具與它的私人連線一起在產品管理的本機環境內啟動，不再要求 Windows 程式連上一個只有 WSL 看得到的位址。第一次開啟時，程式會先確認電腦已經具備什麼，再自動準備本機工具。若掃描器還沒啟動就停止，進度頁會直接說明，提供真正的重新開始，不再誤稱可以接續，並保留可下載的去識別診斷紀錄。程式仍會等使用者明確按下「開始掃描」，才會連線到目標。正式 QC 與程式碼審查仍依規劃留在完整產品流程實測之後進行。
