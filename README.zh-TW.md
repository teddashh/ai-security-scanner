...
> **專案狀態：**`v0.1.3` 是通往 `v0.2.0` 的可安裝公開預覽版，可用來實際測試 Windows 一鍵設定與引導式掃描流程；正式 `v0.2.0` 仍保留給產品完成並經過測試後 QC 的版本。
...
`v0.1.3` 是預定 `v0.2.0` 的公開實測預覽版。安裝程式必須先通過跨平台自動建置與全新主機資格驗證，GitHub 才會發布 pre-release。規格要求的 IAM 新手真人首次使用研究，以及研究之後的正式品質檢查與程式碼審查，仍保留在正式版流程。
