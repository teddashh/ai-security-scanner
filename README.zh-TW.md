...
`v0.1.8` 延續 `v0.1.7` 的 Windows 實機操作修正。第一次開啟若發現本產品舊版本留下的掃描工具工作區，程式會先保留復原備份、自動換成新的工作區，再接著完成設定；過程會直接顯示在畫面上。只有無法確認工作區確實屬於 ai-security-scanner 時，才會顯示 Windows 人工處理說明。既有本機案件不會變更，程式也仍會等使用者明確按下「開始掃描」，才會連線到掃描目標。
