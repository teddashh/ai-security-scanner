...
| Website, authorized `active_external` | Yes | Yes | Limited | Limited | — | Limited | Nuclei runs only the exact pinned and selected HTTP templates. Coverage is limited to those templates and the target actually passed to the launcher. |
...
| Exact host, authorized active profile | Yes | Yes | Limited | Limited | — | Limited | Only checks actually run by selected Nuclei/Greenbone profiles count. The current direct Greenbone profile is restricted to selected `gather_info` NVTs and must not be presented as broad network vulnerability coverage. |
| Source repository / AI-project repository | — | — | Limited | Limited | Yes | Yes | Real secret and recognized dependency findings, a small Semgrep rule set, optional IaC checks, and SBOM inventory. The AI label does not add prompt, model, jailbreak, agent, or live-endpoint testing. |
| IaC working tree | — | — | Limited | Yes | — | Yes | Checkov/KICS provide configuration checks; the current Checkov command is Terraform-only. Trivy contributes only its declared recognized-package profile. |
...
| Active HTTP testing | [Nuclei](https://github.com/projectdiscovery/nuclei) | MIT | Bounded-launcher managed OCI image using only an exact allowlisted template snapshot and explicit `active_external` authorization. Coverage equals the templates actually selected and run. |
| Network scanner | [OpenVAS Scanner](https://github.com/greenbone/openvas-scanner) | GPL-2.0-only; pinned feed also carries GPL/ODbL terms | Managed direct `openvasd` image with a digest-pinned Community Feed snapshot. The current wrapper accepts selected `gather_info` NVTs only; do not claim the breadth of the full Greenbone stack. |
| Source analysis | [Semgrep](https://github.com/semgrep/semgrep) | LGPL-2.1-or-later | Managed OCI image with the project’s small offline rule pack. The current pack covers four narrow patterns: Python `eval/exec`, Python shell execution, JavaScript/TypeScript `child_process.exec`, and private-key material. |
...
| Infrastructure as code | [Checkov](https://github.com/bridgecrewio/checkov) | Apache-2.0 | Managed OCI image with read-only input and external integrations disabled. The current invocation is Terraform-only. |
...
| Container vulnerability | [Grype](https://github.com/anchore/grype) | Apache-2.0 | Managed OCI image with checksum-pinned offline vulnerability data for a backend-validated single-image OCI layout; catalogs OS/language packages including JARs. |
