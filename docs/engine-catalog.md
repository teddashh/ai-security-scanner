...
| Microsoft 365 | [Maester](https://github.com/maester365/maester) | MIT | Managed PowerShell/OCI image that retains source severity and leaves unknown values unknown; published at an immutable digest and independently verified, with the wrapper specification replayed inside the published digest | Short-lived Microsoft Graph read-only permission set | ALLOW |
