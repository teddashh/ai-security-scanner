...
| T-14 | Credentials or findings remain in orphaned containers or temporary files. | Post-run secret and evidence exposure. | Dedicated ephemeral volumes, termination cleanup, startup orphan reconciliation, visible cleanup failures, capability revocation. |
...
| T-19 | Normalization incorrectly merges distinct findings. | A real issue is hidden. | Preserve every source finding and evidence; grouping is reversible; versioned fingerprint; expert access to raw results. |
...
| T-26 | Export redaction omits an unknown sensitive field. | Sensitive disclosure. | Data-classification metadata, allowlisted export schemas, preview and manifest, engine raw output excluded by default when not safely redacted. |
