...
| T-10 | User scans an unauthorized target or an engine is applied to the wrong selected asset. | Legal exposure and harm to third parties. | Per-asset backend scope grants, resolved target allowlists, exact engine-to-asset bindings, tiered activity, expiry, rate limits, immutable run plan, event record. |
...
- a real mixed-asset beginner run covering repositories, internal devices or endpoints, and websites, proving exact engine-to-asset scope, honest inventory-only states, preserved sibling results, and one unified report with findings or an honestly bounded no-findings result; single-target shortcuts should preserve the same semantics;
