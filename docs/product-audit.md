...
- **Integration status:** The current integration source no longer offers a deterministic Repair/Retry loop when the packaged component is missing or rejected. It keeps a redacted, terminal, non-retryable admission outcome; never executes the rejected component; and leaves compatibility providers, independent checks, projects, reports, and exports available. A verified automatic component-repair source and installed-Windows qualification remain open, so this is graceful degradation rather than a claim that automatic Repair is complete.
- **Problem:** If the packaged managed-runtime manifest/bundle fails startup verification, the manager becomes `None` for the process. Execution falls back to user-installed compatibility providers, while scanner-issue UI directs a beginner to fetch/reinstall the latest release.
- **Evidence:** `src-tauri/src/lib.rs:75-98`; fallback at `src-tauri/src/state.rs:79-98`; reinstall guidance at `src/components/RuntimeSetupAssistant.tsx:108-126,224-242`.
- **Real user impact:** One missing/corrupt product-owned component can disable every managed engine until the user diagnoses versions and reinstalls, even though projects and the installer/cache may be intact.
...
- **Acceptance test:** Corrupt or remove one packaged runtime file in an installed Windows fixture. The file is never executed; automatic Repair/relaunch restores the admitted component and returns to the same project, or produces a partial report with only dependent tasks unavailable—never a mandatory download/reinstall journey.
...
- manual finding groups and multi-baseline/analytics comparison workflows; keep the honest single-baseline comparison as a core project capability;
