...
`v0.1.4` must exactly match the versions in `package.json`, `package-lock.json`,
...
[`v0.1.4` actionable scan-flow notes](v0.1.4.md), the planned
...
```sh
npm run release:validate -- --tag v0.1.4
...
```sh
git tag -a v0.1.4 <preflight-head-sha> -m "ai-security-scanner v0.1.4"
