...
Agent assistance is an optional transport over bounded typed product operations, not a separate authority, scanner implementation, or prerequisite for the desktop journey. It may guide installation/status inspection, explain redacted failures, and request an already supported export or exact cleanup inspection. It may not receive credentials, approve or widen scope, contact a target that was not already explicitly authorized by a human, invoke an upstream scanner or arbitrary shell as a substitute for a product adapter, or remediate a target. Human authorization in the desktop/provider flow remains controlling, and failures stay task-scoped.

A checked-in qualification plan may define an agent-operated QA exception only after the owner has explicitly authorized the exact low-impact activity and fixed localhost target in the controlling conversation. The exception is limited to a resettable disposable lab, an ownership-verified `127.0.0.1:9001` fixture, and the same typed product Start operation a human operator could invoke. It never covers credentials, UAC/secure desktop, scope changes, remediation, or destructive all-data cleanup. Its output is operator/regression or installed-lifecycle evidence only; it cannot be presented as beginner-human evidence or satisfy the beginner-human gate.
